...
-- exercicio 4
select f.nome as funcionario, sum(fp.horas_trabalhadas)as horas_trabalhadas
from funcionarios f 
...
--  exercicio 14
select p.nome as projetos,count(fp.id_funcionario) as total_funcionarios 
from projetos p 
left join funcionarios_projetos fp on fp.id_projeto = p.id_projeto 
group by p.nome ;	
	
-- exercicio 15
select p.nome as projeto, f.nome as funcionario, sum(fp.valor_hora * fp.horas_trabalhadas) as valor_pago	
	from funcionarios f 
	inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario 
